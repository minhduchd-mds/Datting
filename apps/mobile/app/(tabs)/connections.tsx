...
export default function Connections() {
  ...
  const load = useCallback(async () => {
    try {
      // CHƯA ai nói gì mới thuộc về đây. Đã có tin nhắn thì thuộc tab Tin
      // nhắn — gộp cả hai làm hỏng cả hai: người mới kết đôi bị chôn dưới các
      // cuộc đang nói dở, còn danh sách tin nhắn thì đầy dòng "chưa có tin".
      setItems((await api.fetchMatches()).filter((m) => m.lastMessage === null));
      setFailed(false);
    } catch {
      setFailed(true);
    }
            ...
            <View style={styles.rowBody}>
              ...
              <Text style={styles.preview} numberOfLines={1}>
                {/* Chưa nhắn gì thì nói thẳng, đừng để dòng trống — dòng trống
                    trông như lỗi tải. */}
                {"Hai bạn đã kết đôi. Nói lời chào nhé."}
              </Text>
            </View>
            {item.unread > 0 && (
